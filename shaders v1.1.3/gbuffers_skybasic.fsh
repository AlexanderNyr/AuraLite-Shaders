...
vec3 computeAtmosphericScattering(vec3 viewDir, vec3 sunDir) {
    ...
    // [v1.1.3] Warm GROUND-BOUNCE weight for multiple scattering. The terrain
    // reflects ~15-20% of incoming light back up (mean planetary albedo, warm
    // soil/veg tint), which re-scatters in the lower atmosphere and shows up
    // as a soft warm floor under the horizon band. Strongest when looking
    // down/along the horizon and when the sun is high; fades at night.
    float groundBounceWeight = mix(0.35, 1.0, 1.0 - clamp(viewDir.y, 0.0, 1.0)) * sunHigh;
    ...
    for (int i = 0; i < 24; ++i) {
        ...
        // [v1.1.3] + warm ground-bounce term (second MS channel). Removes the
        // residual "sterile" look of the horizon the pure blue-bias model had.
        vec3 E_ground = T_sun * vec3(0.21, 0.17, 0.12) * (msBoost * 0.9);
        sumMS += T_view * betaScatP * E_ground / (4.0 * PI) * dt * groundBounceWeight;
    }
