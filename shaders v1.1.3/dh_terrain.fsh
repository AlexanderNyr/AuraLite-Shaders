...
void main() {
    ...
    // [v1.1.3] SOFT SEAM instead of the v1.1.2 hard discard at far+16.
    // The 16 m transition band now stochastically mixes DH LOD fragments with
    // the fully-detailed vanilla chunks (screen-door dithered), so switching
    // render distance or crossing the boundary no longer shows a hard cut.
    // The pattern is static per frame (no time rotation) because DH-only
    // pixels are excluded from TAA history — rotating it would shimmer.
    if (distToCamera < far) {
        discard;
    } else if (distToCamera < far + 16.0) {
        float seamT = (distToCamera - far) / 16.0;
        float door = fract(sin(dot(gl_FragCoord.xy, vec2(12.9898, 78.233))) * 43758.5453);
        if (door > seamT) discard;
    }
