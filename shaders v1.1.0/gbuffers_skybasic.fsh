...
// ==============================================================================
#if SKY_STYLE == 0
const float SUN_SIZE_MULT  = 1.0;     // real 0.004653 rad
const float MOON_SIZE_MULT = 1.0;     // real 0.00476 rad
const float CORONA_MULT    = 0.35;
const float COLOR_SAT_MULT = 1.0;
const float STAR_CORE_BASE = 0.22;
const float STAR_CORE_TOP  = 0.38;
const float SUN_DISK_BRIGHT = 12.0;
#elif SKY_STYLE == 2
// Fantasy: readable sun/moon size (×2.8, like semi-realistic) but huge radiant
// glow, vivid saturated palette and big bright stars for the otherworldly mood.
const float SUN_SIZE_MULT  = 2.8;
const float MOON_SIZE_MULT = 2.8;
const float CORONA_MULT    = 1.4;     // huge radiant glow
const float COLOR_SAT_MULT = 1.40;    // vivid saturated palette
const float STAR_CORE_BASE = 0.45;
const float STAR_CORE_TOP  = 0.70;
const float SUN_DISK_BRIGHT = 16.0;
#else // 1 Semi-realistic (default) — halfway between realistic and the old value
const float SUN_SIZE_MULT  = 1.9;     // between ×1.0 (real) and ×2.8
const float MOON_SIZE_MULT = 1.9;
const float CORONA_MULT    = 0.50;    // between 0.35 and 0.65
const float COLOR_SAT_MULT = 1.08;    // between 1.0 and 1.15
const float STAR_CORE_BASE = 0.26;
const float STAR_CORE_TOP  = 0.44;
const float SUN_DISK_BRIGHT = 13.0;
#endif
...
void main() {
    ...
    if (isOverworld) {
        ...
        // ---- SKY STYLE COLOUR GRADING ----
        // Realistic: no change. Semi-realistic: mild saturation. Fantasy: strong
        // saturation + a magical palette — teal zenith by day, crimson-violet
        // sunsets, deep indigo nights — for a stylised, otherworldly mood.
        #if SKY_STYLE == 2
        {
            float luma = dot(finalSky, vec3(0.2126, 0.7152, 0.0722));
            vec3 sat = mix(vec3(luma), finalSky, 1.40);          // strong saturation
            // Fantasy hue shifts by time of day.
            vec3 dayTint    = mix(sat, sat * vec3(0.78, 1.06, 1.12), 0.35);  // teal-ish
            vec3 sunsetTint = mix(sat, sat * vec3(1.18, 0.55, 0.95) + vec3(0.04, 0.0, 0.05), 0.6); // crimson-violet
            vec3 nightTint  = mix(sat, sat * vec3(0.55, 0.62, 1.18) + vec3(0.01, 0.0, 0.03), 0.45); // indigo
        ...
        {
            float luma = dot(finalSky, vec3(0.2126, 0.7152, 0.0722));
            finalSky = mix(vec3(luma), finalSky, COLOR_SAT_MULT); // mild saturation
        }
        #endif
