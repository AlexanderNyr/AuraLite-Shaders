...
// AuraLite Shaders v1.0.1 - Copyright (c) 2026 AlexanderNyr. Licensed under CC BY-NC-SA 4.0.
...
// [v1.0.0] Falling stars (meteors / shooting stars) crossing the night sky.
...
// ==============================================================================
// [v1.0.0] PHYSICALLY-BASED METEORS (Falling Stars)
// ==============================================================================
...
vec4 sampleMeteor3D(vec3 viewDir, float eventHash, float slotPhaseFrac,
    ...
    vec3 dirHead = (sin((1.0 - u) * theta) * dirTail +
                    sin(u         * theta) * dirHeadMax) / sinTheta;
...
void main() {
    ...
    if (isOverworld) {
        ...
        // [v1.0.0] Physically-based meteors. Visible at night, dimmed by
        ...
        if (dayFactor < 0.9) {
            ...
            // [FIX v1.0.1] Use cos^2(phase/2) to match getMoonPhaseBrightness() in composite.fsh
        ...
        if (worldDir.y > 0.02) {
            ...
            if (visibility > 0.001) {
                ...
                const int steps = 24; // Increased steps for clear vertical pillars
                ...
                for (int i = 0; i < steps; i++) {
                    ...
                    // Domain warping for the sweeping curves
                    float warp = (noise(uv * 0.6 + auroraTime * 0.5) + noise(uv * 1.2 - auroraTime * 0.3) * 0.5) * 3.5;
                    ...
                    // High frequency noise along the ribbon that stretches vertically
                    float rayNoise = noise(vec2(p.x * 0.05 + p.z * 0.05, h * 0.5 - auroraTime * 2.0));
                    float rays = pow(max(0.0, rayNoise), 3.0) * 2.0; // Reduced intensity
                    ...
                    if (density > 0.01) {
                        ...
                        // Mix based on altitude and spatial noise (some curtains are more purple)
                        float colorMix = smoothstep(0.15, 0.7, h); 
                        colorMix += (noise(p.xz * 0.01) * 0.4 - 0.2); // Spatial variation
                        colorMix = clamp(colorMix, 0.0, 1.0);
                        
                        vec3 color = mix(colorGreen, colorPurple, colorMix);
                        
                        // Add a slight deep blue tint to the very top to blend with the sky
                ...
                // Optical depth normalization. Drastically reduced to prevent any blowout 
                // while preserving the shape and saturated colors.
        ...
        // [v1.0.0] Meteors in the End — eternal night, no moon, no atmosphere
