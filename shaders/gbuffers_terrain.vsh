#version 460 compatibility

// ==============================================================================
// AuraLite Shader Pack - Terrain Vertex Shader (GLSL 460 - PBR and FMA Waves)
// ==============================================================================

#define WAVING_LEAVES // [true false]
#define WAVING_GRASS  // [true false]
#define WIND_SPEED 2  // [1 2 3]

// PBR Triggers to tell Iris / OptiFine to bind normal & specular maps!
#define MC_NORMAL_MAP
#define MC_SPECULAR_MAP
#define MC_TEXTURE_FORMAT_LAB_PBR

out vec2 texcoord;
out vec2 lmcoord;
out vec4 glcolor;
out vec3 normal;
out vec3 viewPos;
out vec3 tangent;
out vec3 binormal;

in vec4 mc_Entity;
in vec4 at_tangent; // Tangent vector sent by Iris for normal mapping (LabPBR)
uniform float frameTimeCounter;

void main() {
    texcoord = gl_MultiTexCoord0.xy;
    lmcoord = clamp(gl_MultiTexCoord1.xy * 0.004166667, 0.0, 1.0);
    glcolor = gl_Color;
    
    // Normal & Tangent Space construction for PBR
    normal = normalize(gl_NormalMatrix * gl_Normal);
    tangent = normalize(gl_NormalMatrix * at_tangent.xyz);
    binormal = normalize(cross(normal, tangent) * at_tangent.w);

    vec4 position = gl_Vertex;

    // Waving Foliage animation
    float entityId = mc_Entity.x;
    if (entityId >= 10001.0 && entityId <= 10004.0) {
        float speedFactor = 1.0;
        #if WIND_SPEED == 1
        speedFactor = 0.55;
        #elif WIND_SPEED == 3
        speedFactor = 1.65;
        #endif
        
        float t = frameTimeCounter * 2.2 * speedFactor;
        
        float waveInput = fma(position.x, 1.5, fma(position.z, 1.5, t));
        float wave = sin(waveInput) * 0.06;
        
        float waveInput2 = fma(position.y, 1.2, t * 0.8);
        float wave2 = cos(waveInput2) * 0.04;
        
        #ifdef WAVING_LEAVES
        if (entityId == 10001.0) { // Leaves
            position.x += wave;
            position.y = fma(wave2, 0.4, position.y);
            position.z += wave2;
        }
        #endif

        #ifdef WAVING_GRASS
        if (entityId == 10002.0) { // Grass/Flowers
            position.x = fma(wave, 1.4, position.x);
            position.z = fma(wave2, 1.2, position.z);
        } else if (entityId == 10003.0 || entityId == 10004.0) { // Crops/vines
            position.x = fma(wave, 0.8, position.x);
            position.z = fma(wave2, 0.6, position.z);
        }
        #endif
    }

    vec4 viewPosition = gl_ModelViewMatrix * position;
    viewPos = viewPosition.xyz;
    gl_Position = gl_ProjectionMatrix * viewPosition;
}
