...
float sampleShadow(vec3 shadowScreenPos, float distToCamera, float bias) {
    ...
    // Calculates penumbra size dynamically based on distance from the shadow caster
    // and physically scales with atmospheric weather scattering (Rain & Thunder).
    // Uses 16-sample blocker search and continuous smooth interpolation to ensure
    // 100% stable, flicker-free variable penumbra transitions!
    // ==============================================================================
    float avgBlockerDepth = 0.0;
    float blockerCount = 0.0;
    float searchRadius = 16.0 * texelSize * spreadRadius;
    
    // Sample 16 Poisson Disk points for maximum blocker search stability
    for (int i = 0; i < 16; ++i) {
        vec2 offset = poissonDisk[i * 2] * searchRadius;
        float sampleDepth = texture(shadowtex0, shadowScreenPos.xy + offset).r;
        // Compare with unbiased receiver depth
    ...
    // Physical Light Source Size scaling.
    // Overcast/Rain/Thunder act as a massive scattering medium, transforming the sun/moon 
    // from a point light into a broad, highly diffuse area light source.
    float weatherLightSizeMult = 1.0 + rainStrength * 2.0 + rainStrength * thunderStrength * 2.0;
    float minSpreadFactor = mix(0.15, 0.65, rainStrength * (0.6 + 0.4 * thunderStrength));

    float actualSpread = spreadRadius;
    if (blockerCount > 0.0) {
        avgBlockerDepth /= blockerCount;
        // Physical formula (Thales Theorem): Penumbra = (d_receiver - d_blocker) / d_blocker * LightSize
        ...
        // Continuous smooth interpolation to prevent sudden flickering/instability
        float blockerWeight = blockerCount / 16.0;
        actualSpread = mix(minSpread, pcssSpread, smoothstep(0.0, 1.0, blockerWeight));
    } else {
        // Blocker is extremely close (contact shadow)
    ...
    for (int i = 0; i < 32; ++i) {
        ...
        // Apply bias here to prevent shadow acne
...
void main() {
    ...
    if (depth >= 1.0) {
        ...
        // Moved to gbuffers_skybasic.fsh for reliability; disabled here to avoid double rendering.
        #if 0
        if (isOverworld) {
    ...
    if (isEyeInWater == 1) {
        ...
        // Physical Beer-Lambert water absorption: red light absorbed fast, blue/green preserved
        ...
        // Dynamic ambient light scattering under water
        vec3 waterScatterColor = vec3(0.04, 0.18, 0.42) * (1.0 - waterAbsorption) * (skyLight * 0.8 + 0.2) * (dayFactor * 0.8 + 0.2);
