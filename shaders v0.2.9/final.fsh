...
// ==============================================================================
// SSR — screen-space reflection raymarch (adapted from miniature shader)
// Ref: github.com/mateuskreuch/minecraft-miniature-shader
//      shaders/common/getReflectionColor.fsh
// ==============================================================================
#ifdef SSR
vec4 traceSSR(vec3 viewPos, vec3 R, vec3 N) {
    ...
    float stepScale = 1.3;
    #if SSR_QUALITY == 1
    maxSteps = 14; binarySteps = 4; stepScale = 1.6;
    #elif SSR_QUALITY == 3
    maxSteps = 40; binarySteps = 7; stepScale = 1.1;
    #endif

    // Skip rays going away from camera
    if (R.z >= -0.05) return vec4(0.0);

    float invR    = 1.0 / abs(R.z);
    float invFar2 = 1.0 / (2.0 * far);
    float t       = 1.0;          // start a little off the surface
    vec3 prev     = viewPos;

    for (int i = 0; i < 64; ++i) {
        if (i >= maxSteps) break;

        vec3 cur = viewPos + R * t;
        vec3 scr = viewToScreen(cur);

        if (scr.x < 0.0 || scr.x > 1.0 || scr.y < 0.0 || scr.y > 1.0) break;

        float sd = texture(depthtex0, scr.xy).r;
        float sz = screenToView(scr.xy, sd).z;

        float eps  = 1.0 + 0.1 * clamp(abs(sz) * invFar2, 0.0, 1.0);
        float diff = cur.z - sz * eps;
        ...
        if (diff < 0.0) {
            ...
            if (diff > -16.0) {
                for (int j = 0; j < 8; ++j) {
                    ...
                    float md = texture(depthtex0, ms.xy).r;
                    float mz = screenToView(ms.xy, md).z;
                    if (-mid.z < -mz) a = mid; else b = mid;
                }
            }
            ...
            float hd = texture(depthtex0, hit.xy).r;
            if (hd >= 1.0) break; // sky — fall through

            vec3 color = texture(colortex0, hit.xy).rgb;

            // Proper Schlick Fresnel (water F0 = 0.02)
            vec3 V = normalize(-viewPos);
            float NoV = clamp(dot(N, V), 0.0, 1.0);
            float F0 = 0.02;
            float fresnel = clamp(F0 + (1.0 - F0) * pow(1.0 - NoV, 5.0), 0.05, 1.0);

            return vec4(color, edgeFade(hit.xy) * fresnel);
        }

        prev = cur;
        // Adaptive step
        t += max(stepScale * abs(diff) * invR, 1.0);
    }
    return vec4(0.0);
}
#endif
