...
void main() {
    ...
    if (isOverworld) {
        ...
        // ======================================================================
        // ======================================================================
        // SMOOTH SUN ↔ MOON BLEND (civil twilight)
        // Both contribute simultaneously during dawn/dusk for a natural fade.
        // ======================================================================
        ...
        vec3 sunContrib = vec3(0.0);
        {
            ...
            // Smooth ramp on sun elevation (instead of hard dayFactor switch)
            float sunPresence = smoothstep(-0.06, 0.08, worldL.y) * dayFactor;
        ...
        {
            // Moon direction is opposite the sun's
            float moonElev = -worldL.y;
            float moonPresence = smoothstep(-0.06, 0.08, moonElev) * (1.0 - dayFactor);
            float moonBrightness = getMoonPhaseBrightness(moonPhase);
            float moonK = 4100.0;
            #if MOON_TEMPERATURE == 1
            moonK = 6500.0;  // Cold icy blue
            #elif MOON_TEMPERATURE == 3
            moonK = 3200.0;  // Warm cream / harvest moon
            #endif
        ...
        #elif LIGHT_WRAP == 2
        // Soft wrap (half-Lambert squared) — softens terminator
        float NdotL = pow(max(0.0, rawNdotL * 0.5 + 0.5), 1.6);
        #else
        // Stylized wrap (very soft, stylized look)
        ...
        // Shadow lift (controls how dark unlit areas appear)
        ...
        // ======================================================================
        // SHADOW TINTING - shadowed areas pick up sky ambient colour
        // Real-world shadows under a blue sky are slightly bluish.
        // ======================================================================
        #if SHADOW_TINT != 1
        float shadowDarkness = 1.0 - shadow;
        vec3 shadowTintColor = vec3(0.04, 0.06, 0.12);  // cool blue
        #if SHADOW_TINT == 3
        shadowTintColor = vec3(0.10, 0.06, 0.03);       // warm tint
        #endif
        // Stronger during day, mellow at night
        ...
        if (sunsetFactor > 0.01 && dayFactor > 0.01) {
            ...
            // Smooth wrap so the glow doesn't cut off at the terminator
            float wrap = pow(max(0.0, dot(normal, -L) * 0.5 + 0.5), 2.0);
            float scatter = mix(backDot, wrap, 0.5) * sunsetFactor * dayFactor * skyLight * shadowFactor;
        ...
        // ======================================================================
        // SUN HALO - Mie-like forward scattering on terrain looking near the sun
        // ======================================================================
        #ifdef SUN_HALO
        if (dayFactor > 0.01) {
            ...
            // Sharp forward-scatter lobe (Mie-like)
    ...
    if (roughness > 0.001 || metalness > 0.001) {
        ...
        if (!isOverworld) {
            ...
            // Moon specular — respects MOON_TEMPERATURE & MOON_INTENSITY
