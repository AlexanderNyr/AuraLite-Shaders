...
// ==============================================================================
// AuraLite Shader Pack - Sky Fragment Shader (GLSL 460 - Moon Phases)
// ==============================================================================
...
uniform int dimension; // -1 for Nether, 0 for Overworld, 1 for End
uniform int moonPhase; // Current moon phase: 0 to 7!

// Declare explicit output for modern GLSL 460 compatibility!
layout(location = 0) out vec4 colortex0;

// Scientific Kelvin sunlight conversion
...
// 3D LUNAR SPHERE PHASE CARVER WITH GORGEOUS EARTHSHINE GLOW
float getMoonIllumination(vec2 coord, int phase) {
    float r2 = dot(coord, coord);
    if (r2 > 1.0) return 0.0; // Outside the circle
    
    float diskEdge = smoothstep(1.0, 0.96, r2); // Antialiased edge
    
    // Convert 2D coordinates on the disk to a 3D sphere surface
    float z = sqrt(1.0 - r2);
    vec3 normal3D = vec3(coord.x, coord.y, z);
    
    // Rotate virtual sunlight vector in moon local space based on phase (0 to 7)
    // 0: Full Moon, 1: Waning Gibbous, 2: Third Qtr, 3: Waning Crescent, 
    // 4: New, 5: Waxing Crescent, 6: First Qtr, 7: Waxing Gibbous
    float phaseAngle = float(phase) * 0.78539816; // phase * pi / 4
    vec3 localLightDir = vec3(sin(phaseAngle), 0.0, cos(phaseAngle));
    
    float ndotl = dot(normal3D, localLightDir);
    
    // Apply realistic Earthshine (so dark portion of the moon is faintly visible)
...
void main() {
    ...
    // Organic Dimension Detection based on vanilla sky colors! (100% reliable across all versions)
    ...
    if (isOverworld) {
        ...
        // 360-degree seamless sky gradient: map heights from [-1, 1] to [0, 1] smoothly
        float gradHeight = clamp(dotUp * 0.5 + 0.5, 0.0, 1.0);
        
        vec3 daySky = mix(vec3(0.42, 0.62, 0.9), vec3(0.12, 0.32, 0.72), gradHeight);
        vec3 nightSky = mix(vec3(0.004, 0.006, 0.012), vec3(0.0005, 0.001, 0.003), gradHeight); // 2x Darker
        ...
        // Dynamic weather skies: lighter misty grey for rain, dark stormy grey for thunder!
        vec3 rainSky = vec3(0.24, 0.26, 0.29); // Cozy light misty grey
        vec3 thunderSky = vec3(0.05, 0.055, 0.065); // Dark stormy grey
        vec3 stormSky = mix(rainSky, thunderSky, thunderStrength);
        skyColor = mix(skyColor, stormSky, rainStrength * 0.85);
        
        // 360-degree seamless sky dome (ignore vanilla glcolor in Overworld to prevent black void leaks)
        ...
        if (dayFactor > 0.1) {
            ...
            // --- GORGEOUS PROCEDURAL MOON PHASE RENDERING ---
            float dotMoon = dot(worldDir, worldL);
            float moonGlow = max(0.0, dotMoon);
            
            // Build local tangent space coordinate system for the moon disk
            ...
            // Normalize coordinates by moon angular radius to get a [-1, 1] 2D circle
            vec2 normMoonCoord = moonCoord / 0.028; // 0.028 is the exact moon disk radius
            
            // Carve the moon phase out dynamically
            ...
            glowColor = vec3(0.85, 0.92, 1.0) * (moonMask * 3.5 + moonCorona + moonHalo) * 0.5; // 2x Darker
        }
        
        // Hide sun/moon and skies completely in thunderstorms, partially in standard rain
        glowColor *= (1.0 - rainStrength * mix(0.65, 0.95, thunderStrength));
        
        finalSky = baseSky + glowColor + baseSky * horizonGlow * 0.28;
        
        // Uniform, small, non-stretching twinkling stars in 360 degrees! (Fixed the Z-plane stretching bug)
        if (dayFactor < 0.9) {
            // Scale and sample stars uniformly on the 3D sphere!
            vec3 starDir = normalize(worldDir);
            
            float amountMult = 220.0;
            #if STARS_AMOUNT == 1
            amountMult = 140.0; // Fewer stars
            #elif STARS_AMOUNT == 3
            amountMult = 320.0; // Dense stars
            #endif
            ...
            vec3 stars = vec3(0.92, 0.95, 1.0) * starGlow * twinkle * smoothstep(-0.02, 0.15, worldDir.y) * (1.0 - dayFactor) * (1.0 - rainStrength) * brightMult;
            finalSky += stars;
        }
        
        // Deep cosmic Milky Way nebula (GORGEOUS DIAGONAL AMBER DUST CLOUD with dense star clusters!)
        if (dayFactor < 0.9) {
            // Define the mathematically precise diagonal galactic plane
            ...
            // Soft, highly realistic brownish/amber/bronze space dust color
            vec3 nebulaColor = mix(vec3(0.004, 0.002, 0.008), vec3(0.08, 0.048, 0.022), milkyWay * 0.7) * smoothstep(0.35, 0.75, nebulaNoise);
            
            // Star clusters inside the galactic plane (embedded micro-star dust)
        ...
        // NOTE: Volumetric clouds have been completely removed from gbuffers_skybasic.fsh!
        // There are no unreachable clouds on the skybox anymore.
        // Clouds are now 100% rendered dynamically in world coordinates inside composite.fsh!
