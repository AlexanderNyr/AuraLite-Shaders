...
uniform float frameTimeCounter; // Frame timer
...
in vec3 tangent;
in vec3 binormal;
in vec4 mc_EntityOut; // Block ID passed safely from vertex shader!
...
vec2 getParallaxCoords(vec2 initialTexCoords, vec3 V_tang) {
    ...
    // Raymarching inside the displacement volume (limit loop using maximum iterations based on steps)
    int maxIters = 12;
    #if POM_STEPS == 1
    maxIters = 8;
    #elif POM_STEPS == 3
