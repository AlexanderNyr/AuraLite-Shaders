...
vec2 getParallaxCoords(vec2 initialTexCoords, vec3 V_tang) {
    ...
    // Dynamic layer steps based on angle.
    // More layers are needed at grazing angles; the old mix was reversed and
    // produced the fewest samples exactly where POM needs the most precision.
