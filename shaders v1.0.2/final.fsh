...
// ==============================================================================
// [v1.0.0] Bypass colortex6 entirely. On Iris 1.20 the multi-target write in
...
// [v1.0.1] Photographic (AgX-like) tone mapping.
...
vec3 applyVibrancy(vec3 color, float amount) {
    ...
    // [FIX v1.0.1] Negative amount now properly oversaturates instead of
    // relying on undefined GLSL mix extrapolation + hard clamping.
    return clamp(vec3(luma) + diff * (1.0 - amount), 0.0, 1.0);
}
...
// [v1.0.0] Multi-octave fBm with quintic smoothstep. The combination
...
vec4 traceSSR(vec3 viewPos, vec3 R, vec3 N) {
    ...
    for (int i = 0; i < 64; ++i) {
        ...
        // visible surface the marker currently sits. [FIX v1.0.1] Lowered
...
void main() {
    // [v1.0.1] Early depth fetch for water detection (needed by refraction & SSR).
    ...
    // ==================================================================
    // [v1.0.1] SCREEN-SPACE WATER REFRACTION
    ...
    // ==================================================================
    if (isWaterSurface && isEyeInWater != 1 && depth < 1.0) {
        // --- Reconstruct geometric water normal (4-tap depth derivative) ---
    ...
    if (depth < 1.0) {
        ...
        // ==================================================================
        // [v1.0.0] BULLETPROOF WATER DETECTOR via depth comparison.
        //
        ...
        // ==================================================================
        bool isWaterSurfaceInner = (depthS - depth) > 1e-5;
        if (isWaterSurfaceInner) {
            ...
            if (haveWaterNormal) {
                ...
                // Fallback reconstruction (duplicated for independence from
                // the refraction block, though it should rarely trigger).
                vec2 px = vec2(1.0 / max(viewWidth, 1.0), 1.0 / max(viewHeight, 1.0));
                float d_xp = texture(depthtex0, texcoord + vec2( px.x, 0.0)).r;
                float d_xn = texture(depthtex0, texcoord + vec2(-px.x, 0.0)).r;
                float d_yp = texture(depthtex0, texcoord + vec2( 0.0,  px.y)).r;
    ...
    // [v1.0.1] Cheap single-pass neighbour blur: only pixels whose luminance
