...
void main() {
    ...
    if (isOverworld) {
        ...
        if (dayFactor > 0.1) {
            ...
            extinction = mix(extinction, max(extinction, 0.42), sunsetFactor * 0.55);
        ...
        // Aurora Borealis
        // [FIX v0.2.5] Render auroras directly in skybasic instead of composite only.
        // Some pipelines do not run the composite sky branch for sky geometry, which made
        // auroras invisible in all modes. Also fixes the old reversed smoothstep north mask.
        #if AURORA_MODE != 0
        if (worldDir.y > 0.03) {
            ...
            float auroraTime = frameTimeCounter * 0.22 * speedFactor;
            vec2 auroraUV = worldDir.xz / max(worldDir.y + 0.12, 0.08);

            // Correct north-facing mask: visible in the northern sky, softly fading overhead.
            // Old code used smoothstep(0.0, -0.65, worldDir.z), which is undefined on some GPUs.
            float northFactor = smoothstep(-0.15, 0.70, -worldDir.z);
            northFactor = max(northFactor, 0.22 * smoothstep(0.20, 0.85, worldDir.y));
            ...
            float wave1 = noise(vec2(auroraUV.x * 1.35 + auroraTime, auroraTime * 0.14));
            float wave2 = noise(vec2(auroraUV.x * 2.85 - auroraTime * 0.28, auroraTime * 0.11));
            float curtainA = smoothstep(0.30, 0.70, noise(auroraUV * 0.70 + vec2(wave1, auroraTime * 0.05)));
            float curtainB = smoothstep(0.34, 0.74, noise(auroraUV * 1.45 - vec2(wave2, auroraTime * 0.04)));
            float curtain = curtainA * 0.62 + curtainB * 0.38;

            // Vertical curtain shape: strongest above the horizon, not at zenith.
            float verticalMask = smoothstep(0.05, 0.22, worldDir.y) * (1.0 - smoothstep(0.88, 1.0, worldDir.y));
            ...
            vec3 greenColor = vec3(0.05, 1.00, 0.32);
            vec3 cyanColor = vec3(0.06, 0.72, 1.00);
            vec3 purpleColor = vec3(0.70, 0.12, 1.00);
            vec3 auroraColor = mix(greenColor, cyanColor, smoothstep(0.25, 0.75, worldDir.y));
            auroraColor = mix(auroraColor, purpleColor, smoothstep(0.58, 0.95, worldDir.y) * 0.55);

            float auroraAlpha = curtain * northFactor * verticalMask * nightFactor * (1.0 - rainStrength * 0.85) * auroraBiomeMask * strength;
            finalSky += auroraColor * auroraAlpha;
        }
        #endif
