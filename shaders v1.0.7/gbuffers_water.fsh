...
// ==============================================================================
// AuraLite Shader Pack - Water, Ice & Glass Fragment Shader (GLSL 460 Optimized)
...
// [FIX v0.2.5] Portal pixels use colortex2.a < 0.5 as emissive flag.
// [v1.0.4] Regular ice (ID 10005) now samples gtexture — shows actual ice texture
//          with semi-transparency, NOT water-biome color.
// [v1.0.4] Packed/blue ice (ID 10007) — opaque with texture, no waves.
// [v1.0.4] Glass (ID 10008) — proper texture + transparency, no water behavior.
...
uniform sampler2D gtexture; // [v1.0.4] Added — needed for ice and glass textures
...
in vec3 viewPos;
in float isIce;
in float isRegularIce; // [v1.0.4]
in float isPackedIce;  // [v1.0.4]
in float isGlass;      // [v1.0.4]
in float isPortal;
in float isLava;
...
void main() {
    ...
    if (isLava > 0.5) {
        ...
        vec2 deltaTexCoords = (V_tangent.xy / max(abs(V_tangent.z), 0.15)) * (0.022 / 4.0);
        ...
        float beforeDepth = (1.0 - getLavaHeight(prevTexCoords)) - currentLayerDepth + layerDepth;
        float weight = afterDepth / min(afterDepth - beforeDepth, -1e-5);
        p = mix(currentTexCoords, prevTexCoords, clamp(weight, 0.0, 1.0));
        vec2 offsetUV = texcoord + (p - lavaUV) / 3.5;
        
        // Recalculate cracks and height with displaced coordinates
        float h = getLavaHeight(p);
        float crackStrength = 1.0 - h;
        
        // Base temperature inside cracks (rich glowing orange-red, not washed out)
        float crackNoise = fbm(p * 3.5);
        float temperature = crackStrength * (crackNoise * 0.5 + 0.3);
        
        // Localized hot spots (rare and high-contrast, not everywhere)
        float hotSpotNoise = pow(fbm(p * 1.5), 5.0) * 4.5 * crackStrength;
        ...
        hotLava = mix(hotLava, vec3(1.0, 0.65, 0.05), clamp(hotSpotNoise, 0.0, 1.0));
        // Mix in rare hot yellow-white bubbles
        hotLava = mix(hotLava, vec3(1.0, 0.92, 0.45), clamp(bubbles, 0.0, 1.0));
        
        // Mix basalt crust and lava colors based on height
        vec3 lavaColor = mix(crust, hotLava * 1.8, crackStrength);
    ...
    // ======================================================================
    if (isPortal > 0.5) {
        float t = ((frameTimeCounter) * (0.35) + (0.0));
        vec2 uv = ((texcoord) * (1.5) + (vec2(0.0)));
        float p1 = fbm(uv + vec2(((t) * (0.8) + (0.0)), ((t) * (0.4) + (0.0))));
        float p2 = fbm(((uv) * (1.8) + (vec2(((t) * (-0.5) + (0.0)), ((t) * (0.9) + (0.0))))));
        float plasma = p1 * p2;
        ...
        colortex2 = vec4(((normal) * (0.5) + (0.5)), 0.0); // emissive flag
        return;
    }

    // ======================================================================
    // 2. GLASS (ID 10008) — [v1.0.4] NEW: textured glass rendering
    // ======================================================================
    if (isGlass > 0.5) {
        ...
        colortex1 = vec4(lmcoord, 0.08, 0.0); // glass: smooth, non-metal
        colortex2 = vec4(((normal) * (0.5) + (0.5)), 1.0);
        return;
    }

    // ======================================================================
    // 3. REGULAR ICE (ID 10005) — [v1.0.4] FIXED: textured, semi-transparent
    // ======================================================================
    if (isRegularIce > 0.5) {
        ...
        colortex1 = vec4(lmcoord, 0.12, 0.0); // ice: slightly rough dielectric
        colortex2 = vec4(((normal) * (0.5) + (0.5)), 1.0);
        return;
    }

    // ======================================================================
    // 4. PACKED ICE / BLUE ICE / FROSTED ICE (ID 10007) — [v1.0.4]
    // ======================================================================
    if (isPackedIce > 0.5) {
        ...
        colortex1 = vec4(lmcoord, 0.18, 0.0); // rougher than regular ice
        colortex2 = vec4(((normal) * (0.5) + (0.5)), 1.0);
        return;
    }

    // ======================================================================
    // 5. WATER — original behavior (biome-tinted, wavy, Fresnel)
    // ======================================================================
    ...
    float t = ((frameTimeCounter) * (1.5) + (0.0));
    ...
    waveOffset = vec2(
        ...
        cos(((viewPos.x) * (1.5) + (((viewPos.z) * (-2.5) + (((t) * (0.8) + (0.0)))))))
    ) * riffScale;
