    ...
    if (isOverworld) {
        ...
        if (wetness > 0.01 && rainStrength < 0.98) {
            ...
            if (dayFactor > 0.35) {
                ...
                if (rFactor > 0.001) {
                    float bandPos = clamp((dotRainbow - (rainbowCenter - rainbowWidth)) / (2.0 * rainbowWidth), 0.0, 1.0);
                ...
                if (rFactor2 > 0.001) {
                    float bandPos2 = clamp(1.0 - ((dotRainbow - (rainbowCenter2 - rainbowWidth)) / (2.0 * rainbowWidth)), 0.0, 1.0);
        ...
        // [FIX] Mathematically guaranteed continuous ribbons (no spots/loops) by ensuring the linear
        // phase gradient strictly exceeds the warp gradient. Slowed down movement by 5x.
        // Strengthened horizon mask to completely hide distant aliased waves on the horizon.
        #if AURORA_MODE != 0
        if (worldDir.y > 0.02) {
            ...
            // Снизили скорость в 5 раз для величественного, медленного движения
            float auroraTime = frameTimeCounter * 0.004 * speedFactor;

            // Correct north-facing mask
            ...
            if (visibility > 0.001) {
                ...
                // Smooth starting offset to hide stepping artifacts
                float smoothOffset = noise(worldDir.xz * 100.0);
                ...
                for (int i = 0; i < steps; i++) {
                    ...
                    vec2 uv = p.xz * 0.0015; // Крупный масштаб для плавных, длинных лент
                    
                    // Domain warping (искажение). Используем fbm для органичной формы.
                    float warp = fbm(uv + auroraTime) * 2.0;
                    
                    // Main curtains. 
                    // МАТЕМАТИКА: Градиент линейной части (5.0, 1.5) больше градиента warp (~2.0).
                    // Это гарантирует, что волны будут бесконечными лентами и НИКОГДА не замкнутся в "пятна".
                    // Используем p.z (мировая координата Z) вместо несуществующего uv.z
                    float c1 = 1.0 - abs(sin(uv.x * 5.0 + (p.z * 0.0015) * 1.5 + warp));
                    float c2 = 1.0 - abs(sin(uv.x * 5.0 + (p.z * 0.0015) * 1.5 + warp + 1.2)); // Параллельная лента
                    
                    // Возводим в степень для резких, тонких краев
                    float ribbon = pow(max(0.0, c1), 6.0) + pow(max(0.0, c2), 6.0) * 0.5;
                    
                    // Vertical Pillars / Rays (Striations)
                    float rayNoise = noise(vec2(p.x * 0.02 + p.z * 0.02, h * 0.3 - auroraTime * 3.0));
                    float rays = pow(max(0.0, rayNoise), 2.0) * 2.5; 
                    
                    // Смешиваем ленты и вертикальные лучи
                    float density = ribbon * (0.4 + 0.6 * rays);
                    
                    // Высотный профиль: резкий низ, плавное затухание вверху
                    float heightFade = smoothstep(0.0, 0.08, h) * (1.0 - smoothstep(0.2, 1.0, h));
                    density *= heightFade;
                    
                    if (density > 0.01) {
                        // Фотореалистичные цвета (Cyan-Green и Magenta)
                        vec3 colorGreen  = vec3(0.0, 0.85, 0.45); 
                        vec3 colorPurple = vec3(0.7, 0.1, 0.85); 
                        
                        float colorMix = smoothstep(0.15, 0.7, h); 
                        colorMix += (noise(p.xz * 0.01) * 0.3 - 0.15);
                        colorMix = clamp(colorMix, 0.0, 1.0);
                        
                        vec3 color = mix(colorGreen, colorPurple, colorMix);
                        
                        // Глубокий синий на самых верхушках для слияния с небом
                        vec3 colorBlue = vec3(0.05, 0.1, 0.5);
                        color = mix(color, colorBlue, smoothstep(0.6, 1.0, h));
                ...
                // Сильная маска горизонта. Полностью скрывает искажения ("волны") на горизонте.
                float horizonMask = smoothstep(0.10, 0.28, worldDir.y);
