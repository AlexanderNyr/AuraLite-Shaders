...
// [v1.0.4] fma() fused multiply-add optimizations throughout.
// ==============================================================================
...
vec4 renderVolumetricClouds(vec3 worldDir, vec3 lightDir, vec3 lightColor, float terrainDist, float dayFactor, float sunsetFactor, float t) {
    ...
    float yStart = fma(worldDir.y, t_start, cameraPosition.y);
    float yEnd   = fma(worldDir.y, t_end, cameraPosition.y);
    ...
    float lightPhase = clamp(fma(dot(worldDir, lightDir), 0.5, 0.5), 0.0, 1.0);
    ...
    vec2 windCirrus  = fma(vec2(0.085, 0.034), t * cloudWind, vec2(173.13, -421.70));
    vec2 windAc      = fma(vec2(0.116, 0.071), t * cloudWind, vec2(-317.40, 118.82));
    vec2 windAlto    = fma(vec2(0.145, 0.052), t * cloudWind, vec2(632.20, 274.40));
    vec2 windCumulus = fma(vec2(0.178, 0.083), t * cloudWind, vec2(-127.70, -761.10));
    ...
    for (int i = 0; i < 12; ++i) {
        ...
        vec3 p = fma(worldDir, curr_t, cameraPosition);

        float stepDensity = 0.0;
        vec3 stepColor = vec3(0.0);

        if (p.y >= layerCirrusBottom && p.y <= layerCirrusTop) {
            vec2 uv = fma(cloudCoordCirrus(p.xz), 0.0006, windCirrus);
            ...
            vec2 uv = fma(cloudCoordAc(p.xz), 0.001, windAc);
            float d = fbm(uv * 1.8);
            ...
            vec2 uv = fma(cloudCoordAlto(p.xz), 0.0014, windAlto);
            float d = fbm(uv * 1.5);
            ...
            vec2 uv = fma(cloudCoordCumulus(p.xz), 0.0018, windCumulus);
            float d = fbm(uv);
            ...
            vec3 lightPos = fma(lightDir, 35.0, p);
            float dLight = fbm(fma(cloudCoordCumulus(lightPos.xz), 0.0018, windCumulus));
            ...
            vec3 litCloud = fma(lightColor, 0.92, cloudAmbient * 0.28);
...
float sampleShadow(vec3 shadowScreenPos, float distToCamera, float bias) {
    ...
    for (int i = 0; i < 16; ++i) {
        ...
        if (sampleDepth < shadowScreenPos.z - 1e-4) {
            avgBlockerDepth += sampleDepth;
            blockerCount += 1.0;
        }
    }

    float weatherLightSizeMult = fma(rainStrength, 2.0, fma(rainStrength * thunderStrength, 2.0, 1.0));
...
float sampleCloudLayerDensityForShadow(vec2 xz, float y, float cloudMinY, float cloudSpan, float t) {
    ...
    if (y >= layerAcTop && y <= layerCirrusTop) {
        ...
        float threshold = mix(0.44, 0.28, rainStrength);
        density = smoothstep(threshold, threshold + 0.20, fbm(uv * 1.8)) * 0.32 * (1.0 - rainStrength * 0.25);
    } else if (y >= layerCumulusTop && y < layerAltoTop) {
        // Altostratus: broad soft shading.
        vec2 uv = cloudCoordAlto(xz) * 0.0014 + windAlto;
        float threshold = mix(0.40, 0.22, rainStrength);
        density = smoothstep(threshold, threshold + 0.24, fbm(uv * 1.5)) * 0.42;
    } else {
...
float computeCloudShadow(vec3 worldPos, vec3 lightDir, float dayFactor, float t) {
    ...
    // Three cheap taps through the cloud slab. Different layers contribute with
    // different opacity, so shadows are not a single binary mask.
    float y0 = cloudMinY + cloudSpan * 0.18;
    float y1 = cloudMinY + cloudSpan * 0.46;
    float y2 = cloudMinY + cloudSpan * 0.78;
    float density = 0.0;
    float weightSum = 0.0;

    if (y0 > worldPos.y) {
        float k = (y0 - worldPos.y) / max(lightDir.y, 0.025);
        density += sampleCloudLayerDensityForShadow((worldPos + lightDir * k).xz, y0, cloudMinY, cloudSpan, t) * 0.48;
        weightSum += 0.48;
    }
    if (y1 > worldPos.y) {
        float k = (y1 - worldPos.y) / max(lightDir.y, 0.025);
        density += sampleCloudLayerDensityForShadow((worldPos + lightDir * k).xz, y1, cloudMinY, cloudSpan, t) * 0.34;
        weightSum += 0.34;
    }
    if (y2 > worldPos.y) {
        float k = (y2 - worldPos.y) / max(lightDir.y, 0.025);
        density += sampleCloudLayerDensityForShadow((worldPos + lightDir * k).xz, y2, cloudMinY, cloudSpan, t) * 0.18;
        weightSum += 0.18;
    }
...
float computeGodrays(vec3 rayDirView, vec3 rayDirWorld, float maxRayDistance,
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        float sunVisibility = sampleVolumetricSunVisibility(sampleViewPos);
        #ifdef CLOUD_SHADOWS
        sunVisibility *= computeCloudShadow(sampleWorldPos, lightWorldDir, dayFactor, frameTimeCounter * 0.05);
        #endif
...
void main() {
    ...
    if (isEyeInWater == 1) {
        ...
        // --- 6. SCREEN-SPACE RIPPLE DISTORTION ---
        // Applied in-place: nearby blocks are distorted by animated UV perturbation.
        // This subtle shimmer makes the underwater view feel alive.
        // (We re-read colortex0 with offset UV. Since this runs AFTER lighting, the
        //  fetched texel is already composited — no feedback loop issues.)
        float distortionStrength = 0.0018 * smoothstep(2.0, 12.0, terrainDist);
        ...
        vec3 distortedSample = texture(colortex0, distortedUV).rgb;
        // Blend distorted sample with current color (subtle, not total replacement)
        finalColor = mix(finalColor, distortedSample * waterAbsorption, 0.35);

    } else if (isEyeInWater == 2) {
        // --- LAVA IMMERSION ---
        // Deep orange overlay with pulsating intensity
        float lavaPulse = fma(sin(frameTimeCounter * 1.8), 0.08, 0.92);
