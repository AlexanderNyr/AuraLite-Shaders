...
vec3 ACESFilm(vec3 x) {
    ...
    return clamp((x * fma(a, x, b)) / fma(x, fma(c, x, d), e), 0.0, 1.0);
}
...
vec3 applyVibrancy(vec3 color, float amount) {
    ...
    return clamp(fma(diff, 1.0 - amount, vec3(luma)), 0.0, 1.0);
}
...
void main() {
    // [v1.0.3] Early depth fetch for water detection (needed by refraction & SSR).
