...
// [v1.0.4] fma() fused multiply-add optimizations in POM and math.
...
vec2 getParallaxCoords(vec2 initialTexCoords, vec3 V_tang) {
    ...
    vec2 p = fma(V_tang.xy, parallaxScale, vec2(0.0));
...
void main() {
    ...
    vec3 bumpedNormal = fma(normalMap.xyz, 2.0, -1.0); // [v1.0.4 fma]
    ...
    colortex2 = vec4(fma(finalNormal, 0.5, 0.5), materialMask); // [v1.0.4 fma]
}
