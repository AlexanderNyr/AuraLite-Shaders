...
vec3 kelvinToRGB(float k) {
    ...
    if (k <= 66.0) {
        r = 255.0;
        g = clamp(fma(99.4708025861, log(k), -161.1195681661), 0.0, 255.0);
        if (k < 19.0) {
            b = 0.0;
        } else {
            b = clamp(fma(138.5177312231, log(k - 10.0), -305.0447927307), 0.0, 255.0);
        }
    } else {
...
vec4 sampleMeteor3D(vec3 viewDir, float eventHash, float slotPhaseFrac,
    ...
    vec3 dirHead = fma(sin((1.0 - u) * theta), dirTail,
...
void main() {
    ...
    if (isOverworld) {
        ...
        if (dayFactor < 0.9) {
            ...
            float twinkle = fma(sin(fma(frameTimeCounter, 3.5, starNoise * 80.0)), 0.35, 0.65);
        ...
        if (worldDir.y > 0.02) {
            ...
            if (visibility > 0.001) {
                ...
                for (int i = 0; i < steps; i++) {
                    ...
                    vec2 uv = p.xz * 0.004;
                    uv.x = fma(1.0, auroraTime, uv.x); // Wind drift
                    
                    // Domain warping for the sweeping curves
                    float warp = fma(noise(fma(uv, 0.6, auroraTime * 0.5)) + fma(noise(fma(uv, 1.2, -auroraTime * 0.3)), 0.5, 0.0), 3.5, 0.0);
